...
var ParentUtils = {
  ...
  async _operateRecord(collection, type, msgData, contentMsg) {
    let times, topic;
    ...
    switch (collection | OPS[type]) {
      ...
      case (COLLECTIONS.address | OPS.update): {
        Services.cpmm.sendAsyncMessage("FormAutofill:SaveAddress", msgData);
        break;
      }
      case (COLLECTIONS.address | OPS.remove): {
  ...
  async operateAddress(type, msgData, contentMsg) {
    await this._operateRecord(COLLECTIONS.address, ...arguments);
  },

  async operateCreditCard(type, msgData, contentMsg) {
    await this._operateRecord(COLLECTIONS.creditCard, ...arguments);
  },
